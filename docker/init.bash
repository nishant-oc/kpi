...
# ---------------------------------------------------------------------------
# Add missing columns from original Docker PG9.5 DB before running migrations.
# These columns are missing from the restored DB but required by the new code.
# We add them manually using ADD COLUMN IF NOT EXISTS so this is safe to run
# multiple times and safe on environments where columns already exist.
# ---------------------------------------------------------------------------
echo 'Adding missing columns to match new schema...'
gosu "${UWSGI_USER}" python manage.py shell -c "
from django.db import connection
cursor = connection.cursor()

cursor.execute(\"\"\"ALTER TABLE kpi_asset ADD COLUMN IF NOT EXISTS data_sharing jsonb DEFAULT '{}'::jsonb NOT NULL\"\"\")
print('Ensured kpi_asset.data_sharing exists')

cursor.execute(\"\"\"ALTER TABLE kpi_asset ADD COLUMN IF NOT EXISTS advanced_features jsonb DEFAULT '{}'::jsonb NOT NULL\"\"\")
print('Ensured kpi_asset.advanced_features exists')

cursor.execute(\"\"\"ALTER TABLE kpi_asset ADD COLUMN IF NOT EXISTS searchable_fields text[] DEFAULT '{}'::text[] NOT NULL\"\"\")
print('Ensured kpi_asset.searchable_fields exists')

cursor.execute(\"\"\"ALTER TABLE kpi_asset ADD COLUMN IF NOT EXISTS date_deployed timestamp with time zone NULL\"\"\")
print('Ensured kpi_asset.date_deployed exists')
...
# Insert all migration records that are missing from django_migrations but
# whose schema changes are already present in the DB (either from the
# original Docker DB or from the column additions above).
...
missing_migrations = [
    # kpi — all columns now exist, safe to mark all as applied
